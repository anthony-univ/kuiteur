...
em_aff_debut('Cuiteur | Abonnés', '../styles/cuiteur.css');
em_aff_entete(($_SESSION['usID'] === $_GET['id'] ? "Vos abonnés" : "Les abonnés de {$infosUtilisateurDemandes[1]}"));

em_aff_infos(true, $infos);

echo '<form action="./abonnes.php?id=', crypteSigneURL($_GET['id']),'" method="POST">',
    '<ul>',
        '<li class="pasDeFond">',
...
function aggl_aff_abonnes(mysqli $bd): void {
    ...
    agg_aff_ligne_input_bouton(array('type' => 'submit', 'name' => 'btnValider', 'value' => 'Valider'), 1) : '<tr><td>Vous n\'avez pas d\'abonnés...</td></tr>'),
    '</table>',
    '</form>';
